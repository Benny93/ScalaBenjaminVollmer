...
trait AE {
  ...
  //praseMul2 "multiply 6 by add 4 to 3"  
  def parseMul2(code: String): Option[(Mul, String)] = {
    parseProductOf2(code) match {
      case None => None
      case Some((productOf, afterProductOf)) =>
        parseExp2(afterProductOf) match {
          case None => None
  ...
  def parseBy(code: String): Option[(String, String)] = {
    val strBy = " by "
    if (code.startsWith(strBy))
      Some((strBy, code.drop(strBy.length)))
    else
      None
  }
  ...
  def parseTo(code: String): Option[(String, String)] = {
    val strTo = " to "
    if (code.startsWith(strTo))
      Some((strTo, code.drop(strTo.length)))
    else
      None
  }
